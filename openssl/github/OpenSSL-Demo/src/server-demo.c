...
extern int SSL_set_srv(const int port);
extern int release(int client_sockfd,int server_sockfd,SSL* ssl,SSL_CTX* ctx);
extern int SSL_waiting(SSL **ssl,SSL_CTX **ctx,const int server_sockfd,struct sockaddr_in* client_addr);
extern SSL_CTX* SSL_init(const char* cert_path,const char* privkey_path);
