//DPDK中的memcpy就利用到了SSE/AVX的特点。比较典型的就是
//rte_memcpy内存拷贝函数。内存拷贝是一个非常简单的操作，算法上并
//无难度，关键在于很好地利用处理器的各种并行特性。当前Intel的处理
//器（例如Haswell、Sandy Bridge等）一个指令周期内可以执行两条Load
//指令和一条Store指令，并且支持SIMD指令（SSE/AVX）来在一条指令
//中处理多个数据，其Cache的带宽也对SIMD指令进行了很好的支持。因
//此，在rte_memcpy中，我们使用了平台所支持的最大宽度的Load和Store
//指令（Sandy Bridge为128bit，Haswell为256bit）。此外，由于非对齐的
//存取操作往往需要花费更多的时钟周期，rte_memcpy优先保证Store指令
//存储的地址对齐，利用处理器每个时钟周期可以执行两条Load这个超标
//量特性来弥补一部分非对齐Load所带来的性能损失。更多信息可以参考
//［Ref3-3］。

//在本仓库中的地址为 ： test/x86_64/SIMD-SSE-AVX
...
#include <stdint.h>
#include <string.h>
#include <emmintrin.h> //SSE/AVX/AVX2/AVX512
#include <avxintrin.h>
#include <xopintrin.h>

#ifdef __rtoax_debug__m128i 
