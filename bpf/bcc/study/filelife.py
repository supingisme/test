...
#define FPATH_LEN   DNAME_INLINE_LEN
...
#if 0
        bpf_probe_read_kernel(&data.fname, sizeof(data.fname), d_name.name);
        struct qstr d_pname = d_parent->d_name;
        if (d_pname.len > 0)
            bpf_probe_read_kernel(&data.fpath, sizeof(data.fpath), d_pname.name);
#elif 0
        //while loop inner
        // RUN ERROR
        (void)dentry_path(dentry, data.fpath, FPATH_LEN);
#endif
    }
...
# header
print("%-8s %-6s %-16s %-7s %s %s" % ("TIME", "PID", "COMM", "AGE(s)", "FILE", "PFILE"))

# process event
def print_event(cpu, data, size):
    event = b["events"].event(data)
    print("%-8s %-6d %-16s %-7.2f %s %s" % (strftime("%H:%M:%S"), event.pid,
        event.comm.decode('utf-8', 'replace'), float(event.delta) / 1000,
        event.fname.decode('utf-8', 'replace'),
        event.fpath.decode('utf-8', 'replace')))
