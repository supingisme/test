...
void jemalloc_stats_arenas_i_retained()
{
    unsigned narenas, i;
    size_t mib[4];
    size_t len, miblen;
    ...
    je_mallctlnametomib("stats.arenas.0.retained", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        ...
        /* Do something... */
        printf("stats.arenas.%d.retained = %d bytes,(%d)\n", i, value, len);
    }
}
...
{
    unsigned narenas;
    jemalloc_stats_arenas_i_retained();
