...
void jemalloc_stats_arenas_i_bins_j_nfills()
{
    ...
    je_mallctlnametomib("stats.arenas.0.bins.0.nfills", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        for (j = 0; j < nbins; j++) {
            ...
            /* Do something... */
            printf("stats.arenas.%d.bins.%d.nfills = %d bytes,(%d)\n", i, j, value, len);
        }
    }
}
...
int main()
{
    unsigned narenas;
    jemalloc_test();
    jemalloc_stats_arenas_i_bins_j_nfills();
