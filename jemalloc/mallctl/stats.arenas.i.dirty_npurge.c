...
void jemalloc_stats_arenas_i_dirty_npurge()
{
    unsigned narenas, i;
    size_t mib[4];
    size_t len, miblen;
    ...
    je_mallctlnametomib("stats.arenas.0.dirty_npurge", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        ...
        /* Do something... */
        printf("stats.arenas.%d.dirty_npurge = %d ,(%d)\n", i, value, len);
    }
}
...
{
    unsigned narenas;
    jemalloc_stats_arenas_i_dirty_npurge();
