...
//	Cumulative number of times a bin region of the corresponding size class was allocated from the arena, whether to fill 
...
void jemalloc_stats_arenas_i_bins_j_nmalloc()
{
    ...
    je_mallctlnametomib("stats.arenas.0.bins.0.nmalloc", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        for (j = 0; j < nbins; j++) {
            ...
            /* Do something... */
            printf("stats.arenas.%d.bins.%d.nmalloc = %d bytes,(%d)\n", i, j, value, len);
        }
    }
}
...
{
    unsigned narenas;
    jemalloc_stats_arenas_i_bins_j_nmalloc();
