...
void get_jemalloc_stats_mutexes_ctl_counter()
{
    uint64_t value64 = 1;
    uint32_t value32 = 1;
    ...
    je_mallctl("stats.mutexes.ctl.num_ops", &value64, &size64, NULL, 0);
    printf("num_ops = %ld\n", value64);
    
    je_mallctl("stats.mutexes.ctl.num_spin_acq", &value64, &size64, NULL, 0);
    printf("num_spin_acq = %ld\n", value64);
    
    je_mallctl("stats.mutexes.ctl.num_wait", &value64, &size64, NULL, 0);
    printf("num_wait = %ld\n", value64);
    
    je_mallctl("stats.mutexes.ctl.max_wait_time", &value64, &size64, NULL, 0);
    printf("max_wait_time = %ld\n", value64);
    
    je_mallctl("stats.mutexes.ctl.total_wait_time", &value64, &size64, NULL, 0);
    printf("total_wait_time = %ld\n", value64);
    
    je_mallctl("stats.mutexes.ctl.max_num_thds", &value32, &size32, NULL, 0);
    printf("max_num_thds = %d\n", value32);
    
    je_mallctl("stats.mutexes.ctl.num_owner_switch", &value64, &size64, NULL, 0);
...
{
    get_jemalloc_stats_mutexes_ctl_counter();
}
