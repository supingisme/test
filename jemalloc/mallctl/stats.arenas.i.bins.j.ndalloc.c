...
//	Cumulative number of times a bin region of the corresponding size class was returned to the arena, whether to flush 
//	the relevant tcache if opt.tcache is enabled, or to directly deallocate an allocation otherwise.
...
void jemalloc_stats_arenas_i_bins_j_ndalloc()
{
    ...
    je_mallctlnametomib("stats.arenas.0.bins.0.ndalloc", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        for (j = 0; j < nbins; j++) {
            ...
            /* Do something... */
            printf("stats.arenas.%d.bins.%d.ndalloc = %d bytes,(%d)\n", i, j, value, len);
        }
    }
}
...
{
    unsigned narenas;
    jemalloc_stats_arenas_i_bins_j_ndalloc();
