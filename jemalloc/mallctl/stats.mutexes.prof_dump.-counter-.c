...
//	Statistics on prof dumping mutex (global scope; profiling related). {counter} is one of the counters in mutex 
//	profiling counters.
...
void get_jemalloc_stats_mutexes_prof_dump_counter()
{
    uint64_t value64 = 1;
    uint32_t value32 = 1;
    ...
    je_mallctl("stats.mutexes.prof_dump.num_ops", &value64, &size64, NULL, 0);
    printf("num_ops = %ld\n", value64);
    
    je_mallctl("stats.mutexes.prof_dump.num_spin_acq", &value64, &size64, NULL, 0);
    printf("num_spin_acq = %ld\n", value64);
    
    je_mallctl("stats.mutexes.prof_dump.num_wait", &value64, &size64, NULL, 0);
    printf("num_wait = %ld\n", value64);
    
    je_mallctl("stats.mutexes.prof_dump.max_wait_time", &value64, &size64, NULL, 0);
    printf("max_wait_time = %ld\n", value64);
    
    je_mallctl("stats.mutexes.prof_dump.total_wait_time", &value64, &size64, NULL, 0);
    printf("total_wait_time = %ld\n", value64);
    
    je_mallctl("stats.mutexes.prof_dump.max_num_thds", &value32, &size32, NULL, 0);
    printf("max_num_thds = %d\n", value32);
    
    je_mallctl("stats.mutexes.prof_dump.num_owner_switch", &value64, &size64, NULL, 0);
...
{
    get_jemalloc_stats_mutexes_prof_dump_counter();
}
