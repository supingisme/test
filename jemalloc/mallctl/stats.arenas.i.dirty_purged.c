...
void jemalloc_stats_arenas_i_dirty_purged()
{
    unsigned narenas, i;
    size_t mib[4];
    size_t len, miblen;
    ...
    je_mallctlnametomib("stats.arenas.0.dirty_purged", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        ...
        /* Do something... */
        printf("stats.arenas.%d.dirty_purged = %d,(%d)\n", i, value, len);
    }
}
...
{
    unsigned narenas;
    jemalloc_stats_arenas_i_dirty_purged();
