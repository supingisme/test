...
void jemalloc_stats_arenas_i_lextents_j_curlextents()
{
    ...
    je_mallctlnametomib("stats.arenas.0.lextents.0.curlextents", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        ...
        for (j = 0; j < nlextents; j++) {
            ...
            /* Do something... */
            if(value!=0)
            printf("stats.arenas.%d.lextents.%d.curlextents = %d ,(%d)\n", i, j, value, len);
        }
    }
}
...
int main()
{
    unsigned narenas;
    jemalloc_test();
    jemalloc_stats_arenas_i_lextents_j_curlextents();
