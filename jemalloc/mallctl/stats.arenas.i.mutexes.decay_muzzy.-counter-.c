...
//	Statistics on arena.<i>.decay_muzzy mutex (arena scope; decay for muzzy pages related). {counter} is one of the 
...
void jemalloc_stats_arenas_i_mutexes_decay_muzzy_counter()
{
    ...
    je_mallctlnametomib(CNT_STRING("stats.arenas.0.mutexes.decay_muzzy.", counter_string[0]), mib, &miblen);
    ...
    for (i = 0; i < narenas; i++) {
        ...
        for (k = 0; k < NR_COUNTER; k++) {
            ...
            /* Do something... */
            printf("stats.arenas.%d.mutexes.decay_muzzy.%-20s = %d,(%d)\n", i, counter_string[k], value, len);
        }
    }
}
...
int main()
{
    unsigned narenas;
    jemalloc_test();
    jemalloc_stats_arenas_i_mutexes_decay_muzzy_counter();
...
#define LOOP_COUNTER(name) {    \
        size_t mib[6];          \
        size_t miblen = 6;      \
        je_mallctlnametomib("stats.arenas.0.mutexes.decay_muzzy."#name, mib, &miblen);\
        printf("%s ", #name);       \
        while(miblen){              \
