...
void jemalloc_stats_arenas_i_extents_j_nretained()
{
    ...
    je_mallctlnametomib("stats.arenas.0.extents.0.nretained", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        size_t extent_avail;
        ...
        for (j = 0; j < extent_avail; j++) {
            ...
            /* Do something... */
            printf("stats.arenas.%d.extents.%d.nretained = %d bytes,(%d)\n", i, j, value, len);
        }
    }
}
...
int main()
{
    unsigned narenas;
    jemalloc_test();
    jemalloc_stats_arenas_i_extents_j_nretained();
