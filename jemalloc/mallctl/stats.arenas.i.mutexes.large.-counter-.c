...
//	Statistics on arena.<i>.large mutex (arena scope; large allocation related). {counter} is one of the counters in 
...
void jemalloc_stats_arenas_i_mutexes_large_counter()
{
    ...
    je_mallctlnametomib(CNT_STRING("stats.arenas.0.mutexes.large.", counter_string[0]), mib, &miblen);
    ...
    for (i = 0; i < narenas; i++) {
        ...
        for (k = 0; k < NR_COUNTER; k++) {
            ...
            /* Do something... */
            printf("stats.arenas.%d.mutexes.large.%-20s = %d,(%d)\n", i, counter_string[k], value, len);
        }
    }
}
...
int main()
{
    unsigned narenas;
    jemalloc_test();
    jemalloc_stats_arenas_i_mutexes_large_counter();
...
#define LOOP_COUNTER(name) {    \
        size_t mib[6];          \
        size_t miblen = 6;      \
        je_mallctlnametomib("stats.arenas.0.mutexes.large."#name, mib, &miblen);\
        printf("%s ", #name);       \
        while(miblen){              \
