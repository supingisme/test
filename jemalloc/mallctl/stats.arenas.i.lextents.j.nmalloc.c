...
//	Cumulative number of times a large extent of the corresponding size class was allocated from the arena, whether to 
//	fill the relevant tcache if opt.tcache is enabled and the size class is within the range being cached, or to directly 
...
void jemalloc_stats_arenas_i_lextents_j_nmalloc()
{
    ...
    je_mallctlnametomib("stats.arenas.0.lextents.0.nmalloc", mib, &miblen);
    for (i = 0; i < narenas; i++) {
        ...
        for (j = 0; j < nlextents; j++) {
            ...
            /* Do something... */
            if(value!=0)
            printf("stats.arenas.%d.lextents.%d.nmalloc = %d bytes,(%d)\n", i, j, value, len);
        }
    }
}
...
int main()
{
    unsigned narenas;
    jemalloc_test();
    jemalloc_stats_arenas_i_lextents_j_nmalloc();
