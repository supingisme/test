...
//	Statistics on arena.<i>.extent_avail mutex (arena scope; extent avail related). {counter} is one of the counters in 
...
void jemalloc_stats_arenas_i_mutexes_extent_avail_counter()
{
    ...
    je_mallctlnametomib(CNT_STRING("stats.arenas.0.mutexes.extent_avail.", counter_string[0]), mib, &miblen);
    ...
    for (i = 0; i < narenas; i++) {
        ...
        for (k = 0; k < NR_COUNTER; k++) {
            ...
            /* Do something... */
            printf("stats.arenas.%d.mutexes.extent_avail.%-20s = %d,(%d)\n", i, counter_string[k], value, len);
        }
    }
}
...
int main()
{
    unsigned narenas;
    jemalloc_test();
    jemalloc_stats_arenas_i_mutexes_extent_avail_counter();
...
#define LOOP_COUNTER(name) {    \
        size_t mib[6];          \
        size_t miblen = 6;      \
        je_mallctlnametomib("stats.arenas.0.mutexes.extent_avail."#name, mib, &miblen);\
        printf("%s ", #name);       \
        while(miblen){              \
