...
#define NO_CREATE_PTHREAD_MAIN_TEST
...
#define PTHREADOF(tid) (tid->pthread)

static long latency_threshold = 10000;
nsec_t begin, end;
int fail;

atomic_t flag;
double pass_criteria = 10;
...
{
	struct thread * target_thread = (struct thread *) ((struct thread *)arg)->arg;
	int i, ret;

	debug(DBG_INFO, "Signal sending thread: target thread id =%ld\n",
	      (pthread_t)PTHREADOF(target_thread));
