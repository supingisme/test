...
# warning 我没有找到这个宏定义 R_X86_64_REX_GOTPCRELX
#else
#if defined(__x86_64__)
...
static int load_object_dyn(obj_handle* obj, const char* bin,
  ...
  for (i = 0; i < ehdr->e_phnum; i++) {
    ...
    if (init_array) {
      size_t i;
      for (i = 0; i < init_arraysz / sizeof(void*); i++) {
